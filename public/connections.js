...
export class Connections {
 constructor({onStatus,onApply,onMicTest,isCapturing}){
  ...
  this.voice={device:'',mode:'room',pause:450};
  try{Object.assign(this.voice,JSON.parse(localStorage.getItem('clinical-live-voice')||'{}'));}catch{}
  for(const [id,key] of [['audio-mode','mode'],['audio-pause','pause']])$(id).value=this.voice[key];
  $('audio-device').onchange=()=>this.saveVoice();$('audio-mode').onchange=()=>this.saveVoice();$('audio-pause').onchange=()=>this.saveVoice();
  ...
  $('gemini-preset').onclick=()=>{for(const role of ['assessment','plan','answer','transcription']){$(role+'-provider').value='gemini';$(role+'-model').value=role==='transcription'?'gemini-2.5-flash':'gemini-3.8-flash';this.populate(role);}$('gemini-thinking').value='low';void this.apply();};
  ...
  for(const [role,label] of Object.entries(names)){
   ...
   provider.onchange=()=>{model.value=provider.value==='gemini'?(role==='transcription'?'gemini-2.5-flash':'gemini-3.8-flash'):(role==='transcription'?'gpt-4o-transcribe':'gpt-4.1');this.populate(role);};
 ...
 async apply(){
  ...
  const settings={profile:$('clinical-profile').value,geminiThinking:$('gemini-thinking').value};
 ...
 saveVoice(){
  ...
  this.voice={device:$('audio-device').value,mode:$('audio-mode').value,pause:Number($('audio-pause').value)};
