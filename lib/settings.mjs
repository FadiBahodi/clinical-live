...
export function defaults(env = process.env) {
 ...
 return {profile: 'ed', geminiThinking: env.GEMINI_THINKING || 'low', assessment: {provider, model: env.ASSESSMENT_MODEL || model}, plan: {provider, model: env.PLAN_MODEL || model}, answer: {provider, model: env.ANSWER_MODEL || model}, transcription: {provider: transcriptionProvider, model: transcriptionProvider === 'openai' ? env.TRANSCRIPTION_MODEL || 'gpt-4o-transcribe' : env.GEMINI_TRANSCRIPTION_MODEL || 'gemini-2.5-flash'}};
}
export function validateSettings(value) {
 ...
 return {profile: value.profile, geminiThinking: value.geminiThinking || 'low', ...Object.fromEntries(['assessment','plan','answer','transcription'].map(lane => [lane, {provider:value[lane].provider,model:value[lane].model.replace(/^models\//,'')}]))};
}
