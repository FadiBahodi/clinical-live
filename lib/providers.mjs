...
export async function builtInAdapter(request,{env=process.env,fetcher=fetch,signal,settings=defaults(env),keys=credentials(env)}={}){
 ...
 const selection=settings[request.lane], input=JSON.stringify({utterances:request.segments,previous:request.previous});
 if(selection.provider==='gemini')return gemini([{text:input}],request.instructions,schemas[request.lane],{key:keys.gemini,fetcher,signal,model:selection.model,thinking:settings.geminiThinking||'low',maxTokens:request.lane==='answer'?3072:8192});
 if(!keys.openai)throw new Error('OpenAI is not configured');
 const response=await fetcher('https://api.openai.com/v1/responses',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+keys.openai},signal:timeout(signal),
  body:JSON.stringify({model:selection.model,store:false,instructions:request.instructions,input,max_output_tokens:request.lane==='answer'?3072:8192,text:{format:{type:'json_schema',name:'clinical_'+request.lane,strict:true,schema:schemas[request.lane]}}})
 });
...
export async function listModels(provider,key,{fetcher=fetch,signal}={}){
 ...
 do{
  ...
  const data=await r.json();models.push(...(data.models||[]).filter(m=>m.supportedGenerationMethods?.includes('generateContent')).map(m=>({id:m.name.replace(/^models\//,''),name:m.displayName||m.name,audio:/^models\/gemini-/.test(m.name)&&!/tts|image|robotics|computer-use/.test(m.name)})));token=data.nextPageToken||'';
